...
void handle_enter(uint8_t c){
        screen_y++;
        screen_x = 0;
        if(screen_y == NUM_ROWS)    //if at bottom, scroll up
            scroll_up();
}
void handle_tab(uint8_t c){
           if(screen_x < (NUM_COLS-8))
                screen_x = screen_x + 8;
            else
            {
                screen_y++;
...
void handle_backspace(uint8_t c){
       if(screen_x > 0)            //delete 1 char, same line
            screen_x--;
        else
        {
            if(screen_y > 0)        //go to end of row at previous line only if not at top
            {
                screen_x = NUM_COLS-1;
                screen_y--;
            }
...
void handle_normal_char( uint8_t c){
        //put char into video memory
        *(uint8_t *)(video_mem + ((NUM_COLS * screen_y + screen_x) << 1)) = c;
        *(uint8_t *)(video_mem + ((NUM_COLS * screen_y + screen_x) << 1) + 1) = ATTRIB;
        if(screen_x == NUM_COLS-1)  //if trying to insert at end of line, update cursor placement
        {
            screen_x = 0;
            screen_y++;
            if(screen_y == NUM_ROWS)    //check if at bottom of screen
                scroll_up();
        }
 ...
 * Inputs: void
 * Output: void
 * Functionality: Scrolls the screen
 */
void scroll_up(void)
{
    int i, j;
 ...
 * void update_cusor()
 * Inputs: x coord to update cursor to, y coord to update cursor to 
 * Output: none
 * Functionality: updates the cursor 
 */
